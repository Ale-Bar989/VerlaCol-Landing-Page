    ...
    <div className={`p-4 sm:p-6 md:p-8 ${isDark ? 'bg-gray-900' : 'bg-white'} rounded-xl sm:rounded-2xl max-w-2xl mx-auto`}>
      ...
      <div className="relative w-48 h-48 sm:w-56 sm:h-56 md:w-64 md:h-64 mx-auto mb-6 sm:mb-8">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <Wifi 
            className={`w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 mb-3 sm:mb-4 ${isRunning ? 'text-[#4A5CFF] animate-pulse' : 'text-[#4A5CFF]'}`} 
            strokeWidth={2} 
          />
          <p className={`text-sm sm:text-base font-medium ${isDark ? 'text-gray-400' : 'text-gray-700'}`}>
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-6 md:gap-8 mb-6 sm:mb-8">
        ...
        <div className="text-center">
          <Activity className={`w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 mx-auto mb-2 sm:mb-3 ${isComplete ? 'text-green-500' : isDark ? 'text-gray-600' : 'text-gray-400'}`} strokeWidth={2} />
          ...
          <div className={`text-xs sm:text-sm ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>Ping</div>
        </div>
        ...
        {/* Subida */}
        <div className="text-center">
          <Upload className={`w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 mx-auto mb-2 sm:mb-3 ${isComplete ? 'text-[#7A8FFF]' : isDark ? 'text-gray-600' : 'text-gray-400'}`} strokeWidth={2} />
          <div className={`text-xl sm:text-2xl md:text-3xl font-bold mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
            {isComplete ? Math.floor(targetSpeed * 0.8) : '--'}
          </div>
          <div className={`text-xs sm:text-sm ${isDark ? 'text-gray-500' : 'text-gray-600'}`}><span className="hidden sm:inline">Subida </span>(Mbps)</div>
        </div>
      </div>
