  ...
  ({ children, className = "", icon }) => {
    ...
    return (
      <div className={`relative group h-full ${className}`}>
        ...
        {/* Card content */}
        <div
          className="relative h-full rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 transition-all duration-500 group-hover:translate-y-[-4px]"
          style={{
            background: isDark
        ...
        >
          ...
          {icon && (
            <div
              className="absolute top-4 right-4 sm:top-5 sm:right-5 md:top-6 md:right-6 w-12 h-12 sm:w-13 sm:h-13 md:w-14 md:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center backdrop-blur-sm z-10"
              style={{
                background: isDark
          ...
          <div
            className="absolute inset-0 opacity-[0.02] pointer-events-none rounded-2xl sm:rounded-3xl"
            style={{
              backgroundImage:
                "linear-gradient(rgba(74, 92, 255, 0.5) 1px, transparent 1px), linear-gradient(90deg, rgba(74, 92, 255, 0.5) 1px, transparent 1px)",
